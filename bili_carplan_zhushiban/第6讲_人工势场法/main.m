...
P0 = [0,-d/2,1,1];     % 车辆起点信息，1-2列位置，3-4列速度(横/纵(前进速度))
Pg = [99,d/2,0,0];     % 目标位置
Pobs = [15,7/4,0,0;
    30,-3/2,0,0;
    45,3/2,0,0;
...
len_step = 0.5;          % 步长（定义每一步走多远）过大到不了目标点(振荡)
Num_iter = 200;        % 最大循环迭代次数

%% ***************初始化结束，开始主体循环******************
Pi = P0;               %将车的起始坐标赋给Xi
i = 0;
while sqrt((Pi(1)-P(n,1))^2+(Pi(2)-P(n,2))^2) > 1 % 车辆与目标点的位置 
    i = i + 1;
    Path(i,:) = Pi;    % 保存车走过的每个点的坐标
    
    %计算车辆当前位置与障碍物的单位方向向量、速度向量 
    for j = 1:n-1    
        delta(j,:) = Pi(1,1:2) - P(j,1:2);                              % 用车辆点减障碍点表达斥力
        dist(j,1) = norm(delta(j,:));                                   % 车辆当前位置与障碍物的距离
        unitVector(j,:) = [delta(j,1)/dist(j,1), delta(j,2)/dist(j,1)]; % 斥力的单位方向向量 （未改进前）
    end
    
    %计算车辆当前位置与目标的单位方向向量、速度向量
    delta(n,:) = P(n,1:2)-Pi(1,1:2);                                    %用目标点减车辆点表达引力   
    dist(n,1) = norm(delta(n,:)); 
    unitVector(n,:)=[delta(n,1)/dist(n,1),delta(n,2)/dist(n,1)];        % 引力的单位方向向量
   ...
   %% 计算斥力 
    ...
    for j = 1:n-1
        if dist(j,1) >= d0
            ...
            % 障碍物的斥力1，方向由障碍物指向车辆
            F_rep_ob1_abs = Eta_rep_ob * (1/dist(j,1)-1/d0) * dist(n,1) / dist(j,1)^2; % n=1 大小        
            F_rep_ob1 = [F_rep_ob1_abs*unitVector(j,1), F_rep_ob1_abs*unitVector(j,2)]; % 再加上方向
    ...
    % 增加边界斥力势场，根据车辆当前位置，选择对应的斥力函数 （位置为车头中点，总要保持在绿色区域！！！！）
    if Pi(1,2) > -d+W/2 && Pi(1,2) <= -d/2             %下道路边界区域力场(绿色的下一半)，方向指向y轴正向，并且下面是公路外，因此有速度限制
        F_rep_edge = [0,Eta_rep_edge * norm(Pi(:,3:4))*(exp(-d/2-Pi(1,2)))];
    elseif Pi(1,2) > -d/2 && Pi(1,2) <= -W/2           %下道路分界线区域力场(绿色的上一半)，方向指向y轴负向，并且上面还是公路(超车道)，限制小些(无v)
        F_rep_edge = [0,1/3 * Eta_rep_edge * Pi(1,2).^2];
    % 上道路(超车道)未画出，是白色虚线的上面
    ...
    F_rep = [sum(F_rep_ob(:,1))  + F_rep_edge(1,1),...
           sum(F_rep_ob(:,2)) + F_rep_edge(1,2)];                                      % 所有障碍物(包括边界斥力)的合斥力矢量
...
% 规划出的轨迹有缺陷：
% 曲线到直线的点的曲率由1/R突变到0(但因为方向盘不可能突变，速度又很快,泊车可以)
% 在目标点附近回到了路中间才转向目标点
% 未加动力学约束的轨迹优化
% 人工势场法本身就不完善，需要我们根据需求自己改进！！！
...
Path(i,:)=P(n,:);            %距离目标点小于1米范围时，把路径向量的最后一个点赋值为目标
...
hold on
fill([P0(1),P0(1),P0(1)-L,P0(1)-L],[-d/2-W/2,-d/2+W/2,-d/2+W/2,-d/2-W/2],'b')  %2号车 fill(X,Y,c)
