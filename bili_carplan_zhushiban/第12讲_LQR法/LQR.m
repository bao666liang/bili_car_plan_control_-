...
% 离散控制量u为：[v-d_v;delta-d_delta]
% 根据阿克曼转向原理，计算参考前轮转角 纯跟踪前视点
refPos_Delta = atan(L*refK);

% 参考速度 全程匀速
refSpeed = 40/3.6;

% matlab自带LQR函数：多目标优化，尽快使跟踪偏差趋于零，并使(控制输入)能量损耗小
%% 主程序
% 赋初值
x = refPos_x(1)+0.5; 
y = refPos_y(1)+0.5; 
yaw = refHeading(1)+0.02;%车辆航向角
v = 10;
Delta = 0;% 车辆前轮转角
idx = 1;
...
% 终止条件定义
iter_max = 500;
epsilon = 0.01;% P_old和误差小于0.01停止迭代
